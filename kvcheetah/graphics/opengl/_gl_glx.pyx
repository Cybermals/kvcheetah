"""kvcheetah - OpenGL GLX Backend API"""

#GLX OpenGL Definitions
cdef extern:
    cdef void *glXGetProcAddress(const unsigned char *proc) nogil
...
cdef void *get_gl_func(const char *proc) except *:
    ...
    cdef void *gl_proc = glXGetProcAddress(<const unsigned char*>proc)

    if gl_proc is NULL:
        #Try to get the ARB version of the function
        strcpy(name_buf, proc)
        strcat(name_buf, "ARB")
        gl_proc = glXGetProcAddress(<const unsigned char*>name_buf)
